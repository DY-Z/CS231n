...
def rnn_step_forward(x, prev_h, Wx, Wh, b):
    ...
    N,D = x.shape
    score_x = x.dot(Wx)
    score_h = prev_h.dot(Wh)
    next_h = np.tanh(score_x + score_h + b)
    cache = (x, Wx, prev_h, Wh, b, next_h,D)
    ##############################################################################
    #                               END OF YOUR CODE                             #
    ##############################################################################
    return next_h, cache
...
def rnn_step_backward(dnext_h, cache):
    ...
    ##############################################################################
    x, Wx, prev_h, Wh, b, next_h,D = cache
    dsum = dnext_h*(1-np.square(next_h))
    dx = dsum.dot(Wx.T)
    dWx = x.T.dot(dsum)
...
def rnn_backward(dh, cache):
    ...
    N, T, H = dh.shape
    _,_,_,_,_,_,D = cache["1"]
    dx = np.zeros((N,T,D))
    for i in range(T):
        a, b, c, d, e = rnn_step_backward(dh[:, T-i-1, :]+dh0, cache[str(T-i-1)])
        dx[:,T-1-i,:] += a 
        dh0 = b
        dWx += c 
        dWh += d 
